...
#
# 3. Create a new user:
#    ./npm-api.sh --user-create admin admin@example.com password123 10.1.2.3
#
...
show_help() {
  ...
  echo -e "  --user-create ${COLOR_CYAN}username${CoR} ${COLOR_CYAN}password${CoR} ${COLOR_CYAN}email${CoR} ${COLOR_CYAN}forwardhost${CoR}   Create User with a ${COLOR_YELLOW}username${CoR}, ${COLOR_YELLOW}password${CoR}, ${COLOR_YELLOW}email${CoR} and ${COLOR_YELLOW}forwardhost${CoR}"
...
examples_cli() {
    ...
    echo -e "  $0 --host-create example.com -i 192.168.1.10 -p 8080 -l '[{\"path\":\"/api\",\"forward_scheme\":\"http\",\"forward_port\":8081,\"forward_path\":\"/target/path\"}]'"
    ...
    echo -e "  $0 --user-create john.doe secretpass john.doe@example.com 10.1.2.3"
...
user_create() {
  check_token_notverbose
  if [ -z "$USERNAME" ] || [ -z "$PASSWORD" ] || [ -z "$EMAIL" ] || [ -z "$FORWARDHOST" ]; then
    echo -e "\n 👤 ${COLOR_RED}The --user-create option requires username, password, email and forwardhost.${CoR}"
    echo -e " Usage: ${COLOR_ORANGE}$0 --user-create username password email proxy_host${CoR}"
    echo -e " Example:"
    echo -e "   ${COLOR_GREEN}$0 --user-create john secretpass john@domain.com 10.1.2.3${CoR}\n"
    return 1
  fi
...
user_delete() {
  ...
  check_token_notverbose
    ...
    local FORWARDHOST=$(echo "$RESPONSE" | jq -r '.designated_forward_host')
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --user-create)
            ...
            if [[ $# -lt 4 ]]; then
                echo -e "\n 👤 ${COLOR_RED}The --user-create option requires username, password, email and forwardhost.${CoR}"
                echo -e " Usage: ${COLOR_ORANGE}$0 --user-create <username> <password> <email> <forwardhost>${CoR}"
                echo -e " Example:"
                echo -e "   ${COLOR_GREEN}$0 --user-create john secretpass john@domain.com 10.1.2.3${CoR}\n"
                exit 1
            fi
...
# Actions users
  ...
  user_create "$USERNAME" "$PASSWORD" "$EMAIL" "$FORWARDHOST"
